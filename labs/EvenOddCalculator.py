...
def EvenOddCalculator(args=None):
    try:
        if args is None:
            # If no argument is provided, read from sys.argv
            if len(sys.argv) != 2:
                print('error')
                return
            args = sys.argv[1]
        nums = [int(x.strip()) for x in args.split(',')]
        ...
        nums_center = nums_sorted[1:-1]
        centered = sum(nums_center) // len(nums_center)

        print(f'The sum of all even numbers is {sum(even)}, the sum of all odd numbers is {sum(odd)}, '
              f'the difference between the biggest and smallest number is {diff}, '
              f'the total number of even numbers is {len(even)}, '
              f'the total number of odd numbers is {len(odd)}, '
              f'the centered average is {centered}.')
    except:
